...
def calculate_key_dates(
    ...
    create_and_schedule_options_1_date = distrokid_date + timedelta(days=1)
    create_and_schedule_options_1_dt = make_task_datetime(create_and_schedule_options_1_date)

    review_1a_dt = publish_dt_1a + timedelta(hours=24)
    review_1b_dt = publish_dt_1b + timedelta(hours=24)

    create_and_schedule_options_2_date = review_1b_dt.date() + timedelta(days=1)
    create_and_schedule_options_2_dt = make_task_datetime(create_and_schedule_options_2_date)
    ...
    return {
        ...
        "create_and_schedule_options_1_date": create_and_schedule_options_1_date,
        "create_and_schedule_options_1_dt": create_and_schedule_options_1_dt,
        "review_1a_dt": review_1a_dt,
        "review_1b_dt": review_1b_dt,
        "create_and_schedule_options_2_date": create_and_schedule_options_2_date,
        "create_and_schedule_options_2_dt": create_and_schedule_options_2_dt,
...
def validate_inputs(
    ...
    schedule_options_1_dt = dates["create_and_schedule_options_1_dt"]
    schedule_options_2_dt = dates["create_and_schedule_options_2_dt"]
...
def build_tester_schedule(
    ...
    add_task(
        schedule,
        dates["create_and_schedule_options_1_dt"],
        task_title(song_title, "create, post and schedule option 1a and 1b on TikTok", pivot_version),
        build_description(
            purpose="Create both first-round test reels, upload them to TikTok, and schedule them for their publishing moments.",
            dimensions=[
                "Reels: 1080x1920",
    ...
    add_task(
        schedule,
        dates["review_1a_dt"],
        task_title(song_title, "review option 1a", pivot_version),
        build_description(
    ...
    add_task(
        schedule,
        dates["review_1b_dt"],
        task_title(song_title, "review option 1b and decide winning hook", pivot_version),
        build_description(
    ...
    add_task(
        schedule,
        dates["create_and_schedule_options_2_dt"],
        task_title(song_title, "create, post and schedule option 2a and 2b on TikTok", pivot_version),
        build_description(
            purpose="Create both second-round test reels, upload them to TikTok, and schedule them for their publishing moments.",
            dimensions=[
                "Reels: 1080x1920",
                "Testing cover: 1080x1920",
            ],
    ...
    add_task(
        schedule,
        dates["review_2a_dt"],
        task_title(song_title, "review option 2a", pivot_version),
        build_description(
    ...
    add_task(
        schedule,
        dates["review_2b_dt"],
        task_title(song_title, "review option 2b and decide winning combo", pivot_version),
        build_description(
    ...
    add_task_on_date(
        ...
        task_title(song_title, "crosspost winning combo", pivot_version),
        build_description(
    ...
    add_task_on_date(
        schedule,
        dates["repost_1_date"],
        task_title(song_title, "repost winning combo", pivot_version),
        build_description(
    ...
    add_task_on_date(
        schedule,
        dates["repost_2_date"],
        task_title(song_title, "repost winning combo again", pivot_version),
        build_description(
...
ttk.Label(main_frame, text="Pivot version").grid(row=8, column=2, sticky="w", padx=(50, 0), pady=(0, 5))
pivot_combo = ttk.Combobox(main_frame, width=30, textvariable=pivot_var, values=PIVOT_OPTIONS, state="readonly")
pivot_combo.grid(row=9, column=2, sticky="w", padx=(50, 0), pady=(0, 10))
pivot_combo.bind("<<ComboboxSelected>>", on_pivot_change)

pivot_notes_label = ttk.Label(main_frame, text="Pivot / lessons learned notes")
pivot_notes_label.grid(row=10, column=2, sticky="w", padx=(50, 0), pady=(0, 5))

pivot_notes_box = tk.Text(main_frame, width=42, height=5, wrap="word")
pivot_notes_box.grid(row=11, column=2, rowspan=2, sticky="w", padx=(50, 0), pady=(0, 10))

ttk.Label(main_frame, text="Fixed intervals used").grid(row=13, column=2, sticky="w", padx=(50, 0), pady=(0, 5))
fixed_info = (
    "Create/post/schedule first round after DistroKid: always 1 day\n"
    "Review items: exactly 24 hours after publish datetime\n"
    "Create/post/schedule second round after review 1b: always 1 day\n"
...
ttk.Label(main_frame, text=fixed_info, justify="left").grid(
    row=14, column=2, rowspan=3, sticky="nw", padx=(50, 0), pady=(0, 10)
)
...
result_box = tk.Text(main_frame, width=155, height=26, wrap="word")
