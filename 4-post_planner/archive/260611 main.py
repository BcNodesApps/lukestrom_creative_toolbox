...
def build_caption(song_title: str, hashtags: str) -> str:
    clean_hashtags = hashtags.strip() if hashtags.strip() else ""

    if clean_hashtags:
        return (
            f"Lukestrom - {song_title}\n\n"
...
def build_post_schedule_text(song_title: str, post_items: list[dict], hashtags: str) -> str:
    caption = build_caption(song_title, hashtags)
...
def generate():
    ...
    schedule_txt_path.write_text(build_post_schedule_text(song_title, post_items, hashtags), encoding="utf-8")
...
ttk.Entry(main, width=60, textvariable=song_title_var).grid(row=1, column=0, columnspan=6, sticky="w", pady=(5, 10))

ttk.Label(main, text="Hashtags").grid(row=2, column=0, sticky="w")
hashtags_box = tk.Text(main, width=60, height=5, wrap="word")
hashtags_box.grid(row=3, column=0, columnspan=6, sticky="w", pady=(5, 15))
hashtags_box.insert("1.0", DEFAULT_HASHTAGS)

ttk.Label(main, text="Repost intervals in weeks, relative to initial post").grid(row=4, column=0, columnspan=6, sticky="w", pady=(0, 5))
...
for index, (label, variable) in enumerate(intervals):
    ttk.Label(main, text=label).grid(row=5, column=index, sticky="w", padx=(0, 15))
    ttk.Entry(main, width=8, textvariable=variable).grid(row=6, column=index, sticky="w", padx=(0, 15), pady=(5, 20))
